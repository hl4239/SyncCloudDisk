...
from app.database.models import EpisodesInfo, Movie, MovieType, TMDBInfos, TVCategory, PanCloud, CloudType, \
    MovieCloudInfo
...
async def test_movie():
    ...
    movie.cloud_infos.append(MovieCloudInfo(pancloud=p,pdir_name='2'))
    print(movie)
    await movie_repository.upsert([movie])
    # await  movie_repository.upsert([movie])
