...
class ATMDBIDProviderService(ITMDBIDProvider):
    def __init__(self):
        ...
    @classmethod
    async def get_id_1(cls,title_season:str,movie_type:MovieType,year:Optional[str]=None)->Optional[Tuple[int,int]] :
        """
    ...
    async def get_tmdb_id(self, movie_data_source: MovieDataSourceResult) -> Tuple[int, int]:
        ...
        if  not tmdb_id:
            tmdb_id=await self.get_id_1(title_season=await movie_data_source.original_title, year=await movie_data_source.year,movie_type=await movie_data_source.movie_type)
        if not tmdb_id:
