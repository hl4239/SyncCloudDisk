    ...
    async def _quark_get_dir(cls,pdir_fid:str,quark_client:QuarkParseClient,movie:Movie):
        ...
        if file_list:=ls_resp.get('list'):
            ...
            standardized_results = [StandardizedResult(original_name=f.name, season_number=season_number,is_folder=f.is_folder) for
                                    f in result]
            for f in result:
                f.standardized=lazy(lambda i=f.name:regex_standardizer.get_standardized_result(target_original=i,items=standardized_results))
                if f.is_folder:
    ...
    async def parse_quark(self,prepare_parse_links:PrepareParseLinks):
        ...
        async for quark_link in AsyncMergedCachedIterator([links_quark,scrape_quark_links]):
            ...
            if parse_result.get('ok'):
                ...
                root=ShareFile(type=FileType.FOLDER,name='根',id=pdir_fid,children=lazy(lambda i=quark_parse_client,j=pdir_fid :self._quark_get_dir(j,quark_client=i,movie=movie)))
...
async def main():
    ...
    result=await quark_link_parser.parse_links([PrepareParseLinks(links=[CloudShareLink(url='https://pan.quark.cn/s/4aac6ff8114c')],scrape_quark_links=lazy(lambda : AsyncCachedIterator([])),movie=movie)])
