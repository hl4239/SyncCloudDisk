...
# -------------------------
class JobNextInfo(PydanticBaseModel):
    job_id: str
    task_name: Optional[str] = None
    next_run_at: Optional[datetime] = None            # UTC datetime
    next_run_at_local: Optional[datetime] = None      # localized to scheduler.default_tz


class SchedulerStatusResponse(PydanticBaseModel):
    running: bool
    jobs_count: int
    heap_size: int
    bg_task_active: bool
    next_run: Optional[JobNextInfo] = None
    upcoming: List[JobNextInfo] = []
    tz: str
...
async def update_job(job_id: str, req: JobUpdateRequest):
    ...
    try:
        new_job_id = await scheduler.add_job(
            task_name=req.params.get("task_name", doc.task_name) if isinstance(req.params, dict) and req.params.get("task_name") else doc.task_name,
            cron=new_cron, params=new_params, tz=new_tz, job_id=job_id
        )
    except Exception as e:
        logger.exception("failed to recreate job during update")
...
async def scheduler_status(limit: int = Query(5, ge=1, le=50, description="返回的即将到点任务数量上限")):
    """
    返回调度器内存中当前状态快照（不会进行 DB I/O）。
    调用 CronScheduler.get_status(limit) 获取数据。
    """
    status_dict = await scheduler.get_status(limit=limit)
    # 直接返回字典，FastAPI/Pydantic 会按 SchedulerStatusResponse 做校验/序列化
    return status_dict
