...
# [最终修复版] 修正了使用默认 _id 时 PATCH/DELETE 操作失败的 bug
...
class Filter:
    ...
    def __post_init__(self):
        if self.ops is None: self.ops = ["eq", "lt", "lte", "gt", "gte", "in", "contains", "ne", "exists"]
...
def convert_value_to_bson(field_type: Any, value: Any):
    ...
    # [关键修正] 增加对 date 类型的处理，将其转换为 datetime
    if isinstance(value, date) and not isinstance(value, datetime):
        return datetime.combine(value, time.min)  # 将 date 转换为 datetime

    return value


def generate_filter_schema(doc_model: Type[Document], custom_query_builders: Optional[Dict[str, Callable]] = None) -> \
List[Dict[str, Any]]:
    schema = []
    ...
    def _build_schema_recursive(model_cls: Type, prefix: str = "", label_prefix: str = ""):
        ...
        for field_name, hint in hints.items():
            ...
            if get_origin(base_type) is Annotated:
                ...
                if get_origin(current_type) is Union: current_type = next(
                    (t for t in get_args(current_type) if t is not type(None)), None)
            ...
            if filter_spec:
                ...
                for op in filter_spec.ops:
                    ...
                    item = {"name": f"{field_prefix}__{op}", "field": field_name,
                            ...
                            "options": None}
                    if item_type == "enum" and inspect.isclass(current_type) and issubclass(current_type, Enum): item[
                        "options"] = [e.value for e in current_type]
                    schema.append(item)
...
class GenericRepository:
    ...
    def _parse_id_value(self, raw_value: Any):
        if self.id_field == "_id":
            try:
                ...
                raise ValueError(f"'{raw_value}' is not a valid ObjectId")
    ...
    List[Document]:
        and_clauses, custom_params, generic_params = [], {}, {}
        ...
        if generic_params:
            generic_mongo_filter = build_mongo_filter_from_model(self.model, self.FilterModel(**generic_params))
            if generic_mongo_filter: and_clauses.append(generic_mongo_filter)
        for key, value in custom_params.items():
            builder_func, custom_mongo_filter = self.custom_query_builders[key], None
            sig = inspect.signature(builder_func)
            if len(sig.parameters) == 0:
                if value is True: custom_mongo_filter = builder_func()
            elif value is not None:
                custom_mongo_filter = builder_func(value)
        ...
        if sort_by:
            direction, field = (-1, sort_by[1:]) if sort_by.startswith("-") else (1, sort_by)
